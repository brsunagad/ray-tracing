...
Quad::Quad(const Point& origin, const Vector& span1, const Vector& span2, CoordMapper* texMapper, Material* material):
    p1(origin),p2(origin+span1),p3(origin+span2),p4(origin+span1+span2),span1(span1),span2(span2),normal(cross(span1, span2).normalize()), Solid(texMapper, material)
{
...
Intersection Quad::intersect(const Ray& ray, float previousBestDistance) const {
   ...
   if (quadIntersect)
       return Intersection(quadIntersect.distance,quadIntersect.ray, this, normal, getBaryCoords(quadIntersect.hitPoint()));//replace p1 with bary centric coordinates in future assignments
   
   quadIntersect = tri2->intersect(ray, previousBestDistance);
   if (quadIntersect)
       return Intersection(quadIntersect.distance, quadIntersect.ray, this, normal, getBaryCoords(quadIntersect.hitPoint()));
...
Solid::Sample Quad::sample() const {
    ...
    struct Sample s = { p1 + eps1 * span1 + eps2 * span2, normal };
    return s;
}
